...
namespace sg
{
	...
	bool CollisionManager::Intersect(Collider* left, Collider* right)
	{
		...
		// 두 충돌체 간의 거리와, 각 면적의 절반의 합을 비교해서
		// 거리가 더 길다면 충돌 X, 거리가 더 짧다면 충돌 O
		Vector2 leftSize = left->GetSize();
		Vector2 rightSize = right->GetSize();

		leftPos.x = leftPos.x + leftSize.x / 2;
		leftPos.y = leftPos.y + leftSize.y / 2;

		rightPos.x = rightPos.x + rightSize.x / 2;
		rightPos.y = rightPos.y + rightSize.y / 2;
